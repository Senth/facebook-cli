...
class MySql:
    def __init__(self):
        self.connection = MySQLdb.connect(
            host="localhost", user=MYSQL_USER, passwd=MYSQL_PASSWORD, db=MYSQL_DB
        )
    ...
    def schedule_message(self, scheduled_date, name, message):
        ...
        sql = (
            "INSERT INTO `scheduled` (`scheduled`, `name`, `message`) "
            + "VALUES ("
            + str(scheduled_date)
            + ", '"
            + name
    ...
    def get_messages_to_send(self):
        ...
        sql = (
            "SELECT id, name, message FROM scheduled WHERE scheduled<"
            + MySql._current_time()
            + " AND sent=NULL"
        )
        cursor = self.connection.cursor()
    ...
    def create_db():
        ...
        root_password = getpass(
            prompt="Existing MySQL root password (for creating a new mysql DB): "
        )
        ...
        try:
            ...
            scheduled_table = (
                ...
                + MYSQL_DB
                + "`.`scheduled` ( `id` INT(9) UNSIGNED NOT NULL AUTO_INCREMENT , `scheduled` INT(9) NOT NULL , `name` VARCHAR(50) NOT NULL , `message` TEXT NOT NULL , `sent` INT(9) NULL DEFAULT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB"
            )
